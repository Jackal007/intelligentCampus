...
class Num_Of_1500Calculater(XXCalculater.XXCalculater):
    def setLevel(self):
    ...
    @MyLog.myException
    def calculate(self):
        print("正在计算各个学院获得1500助学金数量")
        i = 1
        while i <= 19:
            
            sql ="SELECT count(*) from score LEFT JOIN subsidy ON score.student_id = subsidy.student_id WHERE college_id = "+ str(i)+ "and stipend = 1500"
            executer.execute(sql)
            e = executer.fetchall()
            
            sql="update college_info set 1500_num = "+str(e)+"where college_id ="+str(i)
