...
class PropotionCalculater1500(XXCalculater.XXCalculater):
    ...
    @MyLog.myException
    def calculate(self):
        print("正在计算该学生所在学院获得1500助学金占比")
        sql = "SELECT SUM(1500_propotion) FROM college_info"
        executer.execute(sql)
        sum1500 = executer.fetchall()
        studentId = str(self.student.getStudentId())
        sql = "select college_id from score where student_id =" + studentId  
        self.executer.execute(sql)
        collegeId = self.executer.fetchall()
        sql = "select 1500_num from college_info where college_id = "+collegeId     
        self.executer.execute(sql)
        s = self.executer.fetchone()[0]/sum1500
        sql = "update students set 1500_propotion ='" + str(s) + "' where student_id=" + studentId
