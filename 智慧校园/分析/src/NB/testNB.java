...
public class testNB {  
    ...
    //  double[][] P_Score=new double[3][2];
     ...
     //PA,PB..分别表示该学生获得A，B,C，D等级奖学金的概率
    ...
    public static int classfiy(String s)
    {
      ...
      public static void testData(){  
          //训练样本    
    	  int A_subsidy=0,B_subsidy=0,C_subsidy=0,D_subsidy=0; //表示获得A,B,C,D等级的数量 
    	  ...
    	  int[] A_library_time_spand={0,0,0,0},B_library_time_spand={0,0,0,0},C_library_time_spand={0,0,0,0};  
    	  int[] A_supermarket_cost_rate={0,0,0,0},B_supermarket_cost_rate={0,0,0,0},C_supermarket_cost_rate={0,0,0,0} ; 
    	  int[] A_dinnerhall_cost_rate={0,0,0,0},B_dinnerhall_cost_rate={0,0,0,0},C_dinnerhall_cost_rate={0,0,0,0};  
    	  ...
    	  for(int j=0;j<list.size();j++){  
    		 ...
    		 // System.out.println(bb.getSubsidy());
    		  if(classfiy(bb.getSubsidy())==1)//未获得助学金
    		  {
    			  A_subsidy++;
    			 ...
    			 /* if(classfiy(bb.getCost_times())==1)
    				  A_cost_time[0]++;
    			  if(classfiy(bb.getCost_times())==2)
    				  B_cost_time[0]++;
    			  if(classfiy(bb.getCost_times())==3)
    				  C_cost_time[0]++;*/
    		  }
    		  else if(classfiy(bb.getSubsidy())==2)//获得1000助学金
    		  {
    			  B_subsidy++;
    			  if(classfiy(bb.getBalance_rank())==1)
    				  A_balance_rank[1]++;
    			  ...
    			  if(classfiy(bb.getScore())==1)
     				 A_score[1]++;
     			 if(classfiy(bb.getScore())==2)
     				 B_score[1]++;
     			 if(classfiy(bb.getScore())==3)
     				 C_score[1]++;
    			  if(classfiy(bb.getCost_amount())==1)
    				  A_cost_amount[1]++;
    			  if(classfiy(bb.getCost_amount())==2)
    		  ...
    		  else if(classfiy(bb.getSubsidy())==3)//获得1500助学金
    		  {
    			  C_subsidy++;
    			  if(classfiy(bb.getBalance_rank())==1)
    				  A_balance_rank[2]++;
    		  ...
    		  else if(classfiy(bb.getSubsidy())==2)//获得2000助学金
    		  {
    			  D_subsidy++;
    			  if(classfiy(bb.getBalance_rank())==1)
    				  A_balance_rank[3]++;
           ...
           // 概率计算，获得分配器
             ...
             PA_subsidy=A_subsidy*1.0/list.size(); // 无法获取助学金的概率  
             PB_subsidy=B_subsidy*1.0/list.size(); //  获取1000助学金的概率 
             PC_subsidy=C_subsidy*1.0/list.size(); //  获取1500助学金的概率 
             PD_subsidy=D_subsidy*1.0/list.size(); //  获取2000助学金的概率 
            	for(int k=0;k<4;++k)
            	{
            		if(k==0)
            		{
            			P_Balance_rank[0][k]=1.0*A_balance_rank[k]/(A_subsidy*1.0)+0.05;
            			...
            			P_Balance_rank[1][k]=1.0*B_balance_rank[k]/(A_subsidy*1.0)+0.05;
            			...
            			P_Balance_rank[2][k]=1.0*C_balance_rank[k]/(A_subsidy*1.0)+0.05;
            		...
            		{
            			P_Balance_rank[0][k]=1.0*A_balance_rank[k]/B_subsidy+0.05;
            			...
            			P_Balance_rank[1][k]=1.0*B_balance_rank[k]/B_subsidy+0.05;
            			...
            			P_Balance_rank[2][k]=1.0*C_balance_rank[k]/B_subsidy+0.05;
            		...
            		{
            			P_Balance_rank[0][k]=1.0*A_balance_rank[k]/C_subsidy+0.05;
            			...
            			P_Balance_rank[1][k]=1.0*B_balance_rank[k]/C_subsidy+0.05;
            			...
            			P_Balance_rank[2][k]=1.0*C_balance_rank[k]/C_subsidy+0.05;
            		...
            		{
            			P_Balance_rank[0][k]=1.0*A_balance_rank[k]/D_subsidy+0.05;
            			...
            			P_Balance_rank[1][k]=1.0*B_balance_rank[k]/D_subsidy+0.05;
            			...
            			P_Balance_rank[2][k]=1.0*C_balance_rank[k]/D_subsidy+0.05;
     ...
     public static void Analyse_test(String id,String a,String b,String c,String d,String e,String f)
     {
    	 ...
    	 if(a.equals("A"))
    	 {
    		 P_Asubsidy*=P_Balance_rank[0][0]; P_Bsubsidy*=P_Balance_rank[0][1];
    		 P_Csubsidy*=P_Balance_rank[0][2]; P_Dsubsidy*=P_Balance_rank[0][3];
       	 }
    	 else if(a.equals("B"))
    	 {
    		 P_Asubsidy*=P_Balance_rank[1][0]; P_Bsubsidy*=P_Balance_rank[1][1];
    		 P_Csubsidy*=P_Balance_rank[1][2]; P_Dsubsidy*=P_Balance_rank[1][3];
       	 }	
    	 else if(a.equals("C"))
    	 {
    		 P_Asubsidy*=P_Balance_rank[2][0]; P_Bsubsidy*=P_Balance_rank[2][1];
    		 P_Csubsidy*=P_Balance_rank[2][2]; P_Dsubsidy*=P_Balance_rank[2][3];
       	 }
    	 
    	 if(b.equals("A"))
    	 {
    		 P_Asubsidy*=P_book_borrow[0][0]; P_Bsubsidy*=P_book_borrow[0][1];
    		 P_Csubsidy*=P_book_borrow[0][2]; P_Dsubsidy*=P_book_borrow[0][3];
       	 }
    	 else if(b.equals("B"))
    	 {
    		 P_Asubsidy*=P_book_borrow[1][0]; P_Bsubsidy*=P_book_borrow[1][1];
    		 P_Csubsidy*=P_book_borrow[1][2]; P_Dsubsidy*=P_book_borrow[1][3];
       	 }	
    	 else if(b.equals("C"))
    	 {
    		 P_Asubsidy*=P_book_borrow[2][0]; P_Bsubsidy*=P_book_borrow[2][1];
    		 P_Csubsidy*=P_book_borrow[2][2]; P_Dsubsidy*=P_book_borrow[2][3];
       	 }	
    	 
    	 if(c.equals("A"))
    	 {
    		 P_Asubsidy*=P_Cost_amount[0][0]; P_Bsubsidy*=P_Cost_amount[0][1];
    		 P_Csubsidy*=P_Cost_amount[0][2]; P_Dsubsidy*=P_Cost_amount[0][3];
       	 }
    	 else if(c.equals("B"))
    	 {
    		 P_Asubsidy*=P_Cost_amount[1][0]; P_Bsubsidy*=P_Cost_amount[1][1];
    		 P_Csubsidy*=P_Cost_amount[1][2]; P_Dsubsidy*=P_Cost_amount[1][3];
       	 }	
    	 else if(c.equals("C"))
    	 {
    		 P_Asubsidy*=P_Cost_amount[2][0]; P_Bsubsidy*=P_Cost_amount[2][1];
    		 P_Csubsidy*=P_Cost_amount[2][2]; P_Dsubsidy*=P_Cost_amount[2][3];
       	 }

    	 
    	 if(d.equals("A"))
    	 {
    		 P_Asubsidy*=P_library_time_spand[0][0]; P_Bsubsidy*=P_library_time_spand[0][1];
    		 P_Csubsidy*=P_library_time_spand[0][2]; P_Dsubsidy*=P_library_time_spand[0][3];
       	 }
    	 else if(d.equals("B"))
    	 {
    		 P_Asubsidy*=P_library_time_spand[1][0]; P_Bsubsidy*=P_library_time_spand[1][1];
    		 P_Csubsidy*=P_library_time_spand[1][2]; P_Dsubsidy*=P_library_time_spand[1][3];
       	 }	
    	 else if(d.equals("C"))
    	 {
    		 P_Asubsidy*=P_library_time_spand[2][0]; P_Bsubsidy*=P_library_time_spand[2][1];
    		 P_Csubsidy*=P_library_time_spand[2][2]; P_Dsubsidy*=P_library_time_spand[2][3];
       	 }
    	 
    	 if(e.equals("A"))
    	 {
    		 P_Asubsidy*=P_library_time[0][0]; P_Bsubsidy*=P_library_time[0][1];
    		 P_Csubsidy*=P_library_time[0][2]; P_Dsubsidy*=P_library_time[0][3];
       	 }
    	 else if(e.equals("B"))
    	 {
    		 P_Asubsidy*=P_library_time[1][0]; P_Bsubsidy*=P_library_time[1][1];
    		 P_Csubsidy*=P_library_time[1][2]; P_Dsubsidy*=P_library_time[1][3];
       	 }	
    	 else if(e.equals("C"))
    	 {
    		 P_Asubsidy*=P_library_time[2][0]; P_Bsubsidy*=P_library_time[2][1];
    		 P_Csubsidy*=P_library_time[2][2]; P_Dsubsidy*=P_library_time[2][3];
       	 }
    	 if(f.equals("A"))
    	 {
    		 P_Asubsidy*=P_Score[0][0]; P_Bsubsidy*=P_Score[0][1];
    		 P_Csubsidy*=P_Score[0][2]; P_Dsubsidy*=P_Score[0][3];
       	 }
    	 else if(f.equals("B"))
    	 {
    		 P_Asubsidy*=P_Score[1][0]; P_Bsubsidy*=P_Score[1][1];
    		 P_Csubsidy*=P_Score[1][2]; P_Dsubsidy*=P_Score[1][3];
       	 }	
    	 else if(f.equals("C"))
    	 {
    		 P_Asubsidy*=P_Score[2][0]; P_Bsubsidy*=P_Score[2][1];
    		 P_Csubsidy*=P_Score[2][2]; P_Dsubsidy*=P_Score[2][3];
       	 }
    	 double max=0.0;
    	 int tag=0;
    	 if(P_Asubsidy>max){tag=1;max=P_Asubsidy;}
    	 if(P_Bsubsidy>max){tag=2;max=P_Bsubsidy;}
    	 if(P_Csubsidy>max){tag=3;max=P_Csubsidy;}
    	 if(P_Dsubsidy>max)tag=4;
    	 if(tag==1)
    	 {
//    		 System.out.println("学号为 "+id+" 无法获得助学金的可能性较大");
    		 System.out.println(id+" 0");
    	 }
    	 else if(tag==2)
    	 {
//    		 System.out.println("学号为 "+id+" 获得1000元助学金的可能性较大");
    		 System.out.println(id+" 1000");
    	 }
    	 else if(tag==3){
//    		 System.out.println("学号为 "+id+" 获得1500元助学金的可能性较大");
    		 System.out.println(id+" 1500");
    	 }
    	 else{
//    		 System.out.println("学号为 "+id+" 获得2000元助学金的可能性较大");
    		 System.out.println(id+" 2000");
    	 }
    	 P_Asubsidy=1.0;P_Bsubsidy=1.0;
    	 P_Csubsidy=1.0;P_Dsubsidy=1.0;
     }
     public static void analyse()
     {
     	...
     	//Analyse_test("1","A","A","A","B","B","B");
     	 for(int i=0;i<test.size();++i)
     	 {
     		 ...
     		 Analyse_test(stu.getStudent_id(),stu.getBalance_rank(),stu.getBook_borrow(),stu.getScore(),stu.getCost_amount(),stu.getLibrary_time_spand(),stu.getLibrary_times());
